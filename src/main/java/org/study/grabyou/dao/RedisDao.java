...
public class RedisDao {
  ...
  public void addBlackListRecord(BlackRecord blackRecord) {
    if (blackRecord.getType() == EventType.REGISTER.getValue()) {
      if (!StringUtils.isEmpty(blackRecord.getUsername())) {
        redisTemplate.opsForSet().add(REGISTER_BLACKLIST_USERNAME, blackRecord.getUsername());
      }
    ...
    if (blackRecord.getType() == EventType.LOGIN.getValue()) {
      if (!StringUtils.isEmpty(blackRecord.getUsername())) {
        redisTemplate.opsForSet().add(LOGIN_BLACKLIST_USERNAME, blackRecord.getUsername());
      }
      if (blackRecord.getDimension() == DimensionType.TELEPHONE.getValue()) {
    ...
    if (blackRecord.getType() == EventType.VERIFY.getValue()) {
      if (!StringUtils.isEmpty(blackRecord.getUsername())) {
        redisTemplate.opsForSet().add(VERIFYCODE_BLACKLIST_USERNAME, blackRecord.getUsername());
      }
      if (blackRecord.getDimension() == DimensionType.TELEPHONE.getValue()) {
  ...
  public boolean containBlackList(Event event) {
    ...
    if (event.getEventType().getValue() == EventType.REGISTER.getValue()) {
      if (!StringUtils.isEmpty(event.getTelephoneNumber())) {
        res = redisTemplate.opsForSet().members(REGISTER_BLACKLIST_TELEPHONE)
            .contains(event.getTelephoneNumber());
        if (res) {
          return res;
        }
      }
      if (!StringUtils.isEmpty(event.getUserName())) {
        res = redisTemplate.opsForSet().members(REGISTER_BLACKLIST_USERNAME)
            .contains(event.getUserName());
        if (res) {
          return res;
        }
      }
      ...
      res = redisTemplate.opsForSet().members(REGISTER_BLACKLIST_DEVICE)
          .contains(event.getOperateDeviceID());
      if (res) {
        return res;
      }
    }
    ...
    if (event.getEventType().getValue() == EventType.LOGIN.getValue()) {
      if (!StringUtils.isEmpty(event.getTelephoneNumber())) {
        res = redisTemplate.opsForSet().members(LOGIN_BLACKLIST_TELEPHONE)
            .contains(event.getTelephoneNumber());
        if (res) {
          return res;
        }
      ...
      if (!StringUtils.isEmpty(event.getUserName())) {
        res = redisTemplate.opsForSet().members(LOGIN_BLACKLIST_USERNAME)
            .contains(event.getUserName());
        if (res) {
          return res;
        }
      ...
      res = redisTemplate.opsForSet().members(LOGIN_BLACKLIST_DEVICE)
          .contains(event.getOperateDeviceID());
      if (res) {
        return res;
      }
    ...
    if (event.getEventType().getValue() == EventType.VERIFY.getValue()) {
      if (!StringUtils.isEmpty(event.getTelephoneNumber())) {
        res = redisTemplate.opsForSet().members(VERIFYCODE_BLACKLIST_TELEPHONE)
            .contains(event.getTelephoneNumber());
        if (res) {
          return res;
        }
      ...
      if (!StringUtils.isEmpty(event.getUserName())) {
        res = redisTemplate.opsForSet().members(VERIFYCODE_BLACKLIST_USERNAME)
            .contains(event.getUserName());
        if (res) {
          return res;
        }
      }
      res = redisTemplate.opsForSet().members(VERIFYCODE_BLACKLIST_IP)
          .contains(event.getOperateIp());
      if (res) {
        return res;
      }
      res = redisTemplate.opsForSet().members(VERIFYCODE_BLACKLIST_DEVICE)
          .contains(event.getOperateDeviceID());
      if (res) {
        return res;
      }
...
//  }
  ...
  public <T> T scriptLoad(final String script) {
    return (T) redisTemplate.execute(
        (RedisCallback<Object>) connection -> ((Jedis) connection.getNativeConnection())
            .scriptLoad(script));
  }
